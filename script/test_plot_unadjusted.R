...
load("data-raw/RObject/lnorm_test_unadjusted_rf.RData")
load("data-raw/RObject/test_direct_lnorm_para.RData")
load("data-raw/RObject/lnorm_test_unadjusted_gbm.RData")
load("data-raw/RObject/lnorm_test_unadjusted_svr.RData")

lnorm_test_unadjusted_rf <- lnorm_test_unadjusted_rf %>%
  mutate(
...
lnorm_test_unadjusted_svr <- lnorm_test_unadjusted_svr %>%
  mutate(
...
lnorm_test_unadjusted_gbm <- lnorm_test_unadjusted_gbm %>%
  mutate(
...
comb_para_lnorm <- inner_join(
  x = test_direct_lnorm_para, y = lnorm_test_unadjusted_gbm,
  by = c("ID")
)
...
comb_para_lnorm <- inner_join(
  x = comb_para_lnorm, y = lnorm_test_unadjusted_svr,
  by = c("ID")
)

comb_para_lnorm <- inner_join(
  x = comb_para_lnorm, y = lnorm_test_unadjusted_rf,
  by = c("ID")
)
...
data_long_unadjusted = data_long %>%
  mutate(condition = "Unadjusted")

save(data_long_unadjusted, file = "data-raw/Robject/data_long_unadjusted.RData")
