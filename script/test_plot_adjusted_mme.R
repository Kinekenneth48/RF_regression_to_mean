...
load("data-raw/RObject/test_svr_lnorm_para_mme.RData")
load("data-raw/RObject/test_gbm_lnorm_para_mme.RData")
load("data-raw/RObject/test_rf_lnorm_para_mme.RData")

# append station id
ID <- rownames(test_svr_lnorm_para_mme)
test_svr_lnorm_para_mme <- cbind(ID, test_svr_lnorm_para_mme)

ID <- rownames(test_rf_lnorm_para_mme)
test_rf_lnorm_para_mme <- cbind(ID, test_rf_lnorm_para_mme)

ID <- rownames(test_gbm_lnorm_para_mme)
test_gbm_lnorm_para_mme <- cbind(ID, test_gbm_lnorm_para_mme)
...
test_svr_lnorm_para_mme <- as.data.frame(test_svr_lnorm_para_mme) %>%
  mutate(
...
test_rf_lnorm_para_mme <- as.data.frame(test_rf_lnorm_para_mme) %>%
  mutate(
...
test_gbm_lnorm_para_mme <- as.data.frame(test_gbm_lnorm_para_mme) %>%
  mutate(
...
comb_para_lnorm <- inner_join(
  x = test_direct_lnorm_para_mme, y = test_svr_lnorm_para_mme,
  by = c("ID")
)


comb_para_lnorm <- inner_join(
  x = comb_para_lnorm, y = test_gbm_lnorm_para_mme,
  by = c("ID")
)


comb_para_lnorm <- inner_join(
  x = comb_para_lnorm, y = test_rf_lnorm_para_mme,
  by = c("ID")
)
