#############################################################################
# This script is created to bias correct the GEV distr.'s spread parameter
# using the distfixer package created. The data used here is the SWE data used 
# in Jess's paper. Moment matching estimation is used in this script for GEV estimation
##############################################################################
...
test_direct_lnorm_para_mme <- do.call(rbind, lapply(
  X = ls_df_test,
  FUN = fit_lnorm, column = "maxv_WESD", method = "mme"
))

save(test_direct_lnorm_para_mme, file = "data-raw/RObject/test_direct_lnorm_para_mme.RData")
...
# ===========================================================================#
lnorm_test_unadjusted_rf_mme <- do.call(
  rbind,
  lapply(
    X = ls_test_unadjusted, FUN = fit_lnorm, method = "mme",
    column = "pred_load"
  )
)

save(lnorm_test_unadjusted_rf_mme, file = "data-raw/RObject/lnorm_test_unadjusted_rf_mme.RData")
...
train_rf_per <- future_lapply(
  ...
  multiplier = "maxv_SNWD", method = "mme",
...
# get the percentiles
test_rf_lnorm_para_mme <- do.call(rbind, future_lapply(
  ...
  indirect_label = "maxv_WESD", method = "mme",
  future.scheduling = 2, future.seed = TRUE,
  future.packages = c("tidyverse", "distfixer")
))
## Shut down parallel workers
...
save(test_rf_lnorm_para_mme, file = "data-raw/RObject/test_rf_lnorm_para_mme.RData")
...
# ===========================================================================#
lnorm_test_unadjusted_gbm_mme <- do.call(
  rbind,
  lapply(
    X = ls_test_unadjusted, FUN = fit_lnorm, method = "mme",
    column = "pred_load"
...
save(lnorm_test_unadjusted_gbm_mme, file = "data-raw/RObject/lnorm_test_unadjusted_gbm_mme.RData")
...
# get the percentiles
test_gbm_lnorm_para_mme <- do.call(rbind, future_lapply(
  X = ls_df_test, FUN = distfixer::predict_param,
  direct_label = "RATIO", bounds = c(0.0001, Inf),
  fitted_model = fit_gbm, mean = mean_gbm, sd = sd_gbm,
  nboot = 500, distr = "lnorm", all_missing = TRUE,
...
save(test_gbm_lnorm_para_mme, file = "data-raw/RObject/test_gbm_lnorm_para_mme.RData")
...
# fit gev on test data without bais correction
# ===========================================================================#
lnorm_test_unadjusted_svr_mme <- do.call(
  rbind,
  lapply(
    X = ls_test_unadjusted,
    FUN = fit_lnorm, method = "mme",
    column = "pred_load"
  )
)

save(lnorm_test_unadjusted_svr_mme, file = "data-raw/RObject/lnorm_test_unadjusted_svr_mme.RData")
...
# get the percentiles
test_svr_lnorm_para_mme <- do.call(rbind, future_lapply(
  X = ls_df_test, FUN = distfixer::predict_param,
  direct_label = "RATIO", bounds = c(0.0001, Inf),
  fitted_model = fit_svr, mean = mean_svr, sd = sd_svr,
  nboot = 500, distr = "lnorm", all_missing = TRUE,
...
save(test_svr_lnorm_para_mme, file = "data-raw/RObject/test_svr_lnorm_para_mme.RData")
