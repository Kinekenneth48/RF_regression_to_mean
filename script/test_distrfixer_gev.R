...
test_direct_gev_para <- do.call(rbind, lapply(
  X = ls_df_test,
  FUN = fit_gev, column = "maxv_WESD"
))

save(test_direct_gev_para, file = "data-raw/RObject/test_direct_gev_para.RData")
...
# ===========================================================================#
gev_test_unadjusted_rf = do.call(rbind, 
                         lapply(X = ls_test_unadjusted , FUN = fit_gev, 
                                column = "pred_load" ))

save(gev_test_unadjusted_rf, file = "data-raw/RObject/gev_test_unadjusted_rf.RData")
...
train_rf_per <- future_lapply(
  ...
  nboot = 500, distr = "gev",
  param_adjust = "scale", label_convert = TRUE,
...
# get the percentiles
test_rf_gev_para <- do.call(rbind, future_lapply(
  X = ls_df_test, FUN = distfixer::predict_param,
  direct_label = "RATIO", bounds = c(0.0001, Inf),
  fitted_model = fit_rf, mean = mean_rf, sd = sd_rf,
  nboot = 500, distr = "gev", all_missing = TRUE,
  param_adjust = "scale", label_convert = TRUE,
...
save(test_rf_gev_para, file = "data-raw/RObject/test_rf_gev_para.RData")
...
# fit gev on test data without bais correction
# ===========================================================================#
gev_test_unadjusted_gbm = do.call(rbind, 
                                 lapply(X = ls_test_unadjusted , FUN = fit_gev, 
                                        column = "pred_load" ))

save(gev_test_unadjusted_gbm, file = "data-raw/RObject/gev_test_unadjusted_gbm.RData")
...
# get the percentiles
test_gbm_gev_para <- do.call(rbind, future_lapply(
  X = ls_df_test, FUN = distfixer::predict_param,
  direct_label = "RATIO", bounds = c(0.0001, Inf),
  fitted_model = fit_gbm, mean = mean_gbm, sd = sd_gbm,
  nboot = 500, distr = "gev", all_missing = TRUE,
...
save(test_gbm_gev_para, file = "data-raw/RObject/test_gbm_gev_para.RData")
...
# fit gev on test data without bais correction
# ===========================================================================#
gev_test_unadjusted_svr = do.call(rbind, 
                                 lapply(X = ls_test_unadjusted , FUN = fit_gev, 
                                        column = "pred_load" ))

save(gev_test_unadjusted_svr, file = "data-raw/RObject/gev_test_unadjusted_svr.RData")
...
# get the percentiles
test_svr_gev_para <- do.call(rbind, future_lapply(
  X = ls_df_test, FUN = distfixer::predict_param,
  direct_label = "RATIO", bounds = c(0.0001, Inf),
  fitted_model = fit_svr, mean = mean_svr, sd = sd_svr,
  nboot = 500, distr = "gev", all_missing = TRUE,
...
save(test_svr_gev_para, file = "data-raw/RObject/test_svr_gev_para.RData")
