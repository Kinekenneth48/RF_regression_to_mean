...
param_adjusted_gaussian_1k <- matrix(nrow = bootstraps, ncol = 2)
colnames(param_adjusted_gaussian_1k) <- c("mean", "sd")


param_actual_gaussian_1k <- matrix(nrow = bootstraps, ncol = 2)
colnames(param_actual_gaussian_1k) <- c("mean", "sd")

param_unadjusted_gaussian_1k <- matrix(nrow = bootstraps, ncol = 2)
colnames(param_unadjusted_gaussian_1k) <- c("mean", "sd")
...
for (i in 1:bootstraps) {
  ...
  # data creation
  # =============================================================================#
  data <- simulate_gaussian_data(sample_size = 1000, error_sd = 2)
  ...
  param_adjusted_gaussian_1k[i, ] <- distfixer::predict_param(
  ...
  param_actual_gaussian_1k[i, ] <- fitdistrplus::fitdist(
    data = test_data$y,
    distr = "norm"
  )[["estimate"]]
  ...
  param_unadjusted_gaussian_1k[i, ] <- fitdistrplus::fitdist(
    data = pred_y,
    distr = "norm"
  )[["estimate"]]
}
...
save(param_unadjusted_gaussian_1k, file = "data-raw/RObject/param_unadjusted_gaussian_1k.RData")
save(param_actual_gaussian_1k, file = "data-raw/RObject/param_actual_gaussian_1k.RData")
save(param_adjusted_gaussian_1k, file = "data-raw/RObject/param_adjusted_gaussian_1k.RData")
...
#   dplyr::mutate(
#     bias_mean_adj = mean_adjusted - mean,
#     bias_sd_adj = sd_adjusted - sd,
#     bias_mean_unadj = mean_unadjusted - mean,
#     bias_sd_unadj = sd_unadjusted - sd,
#     cov_actual = sd/mean,
#     cov_adjust = sd_adjusted/ mean_adjusted,
#     cov_unadjust = sd_unadjusted/mean_unadjusted
#   ) %>%
#   dplyr::mutate(
#     bias_reduction_sd = (bias_sd_unadj - bias_sd_adj) / bias_sd_unadj,
#     bias_reduction_mean = (bias_mean_unadj - bias_mean_adj) / bias_mean_unadj
#   )
