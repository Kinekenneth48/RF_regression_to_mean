#############################################################################
# This script is created to simulate data (10,000 obs. per sim) from a uniform model,
...
param_adjusted_runif_10k <- matrix(nrow = bootstraps, ncol = 2)
colnames(param_adjusted_runif_10k) <- c("mean", "sd")


param_actual_runif_10k <- matrix(nrow = bootstraps, ncol = 2)
colnames(param_actual_runif_10k) <- c("mean", "sd")

param_unadjusted_runif_10k <- matrix(nrow = bootstraps, ncol = 2)
colnames(param_unadjusted_runif_10k) <- c("mean", "sd")
...
for (i in 1:bootstraps) {
  ...
  x1 <- runif(n = 10000, min = -1, max = 1)
  x2 <- runif(n = 10000, min = -1, max = 1)
  x3 <- runif(n = 10000, min = -1, max = 1)
  x4 <- runif(n = 10000, min = -1, max = 1)
  x5 <- runif(n = 10000, min = -1, max = 1)

  error <- rnorm(10000, 0, 2)
  ...
  param_adjusted_runif_10k[i, ] <- distfixer::predict_param(
  ...
  param_actual_runif_10k[i, ] <- fitdistrplus::fitdist(
    data = test_data$y,
    distr = "norm"
  )[["estimate"]]
  ...
  param_unadjusted_runif_10k[i, ] <- fitdistrplus::fitdist(
    data = pred_y,
    distr = "norm"
  )[["estimate"]]
}
...
save(param_unadjusted_runif_10k, file = "data-raw/RObject/param_unadjusted_runif_10k.RData")
save(param_actual_runif_10k, file = "data-raw/RObject/param_actual_runif_10k.RData")
save(param_adjusted_runif_10k, file = "data-raw/RObject/param_adjusted_runif_10k.RData")
...
column_means <- colMeans(comb_para_runif_10k)
