...
param_adjusted_runif <- matrix(nrow = bootstraps, ncol = 2)
colnames(param_adjusted_runif) <- c("mean", "sd")


param_actual_runif <- matrix(nrow = bootstraps, ncol = 2)
colnames(param_actual_runif) <- c("mean", "sd")

param_unadjusted_runif <- matrix(nrow = bootstraps, ncol = 2)
colnames(param_unadjusted_runif) <- c("mean", "sd")
...
for (i in 1:bootstraps) {
  ...
  param_adjusted_runif[i, ] <- distfixer::predict_param(
  ...
  param_actual_runif[i, ] <- fitdistrplus::fitdist(
    data = test_data$y,
    distr = "norm"
  )[["estimate"]]
  ...
  param_unadjusted_runif[i, ] <- fitdistrplus::fitdist(
    data = pred_y,
    distr = "norm"
  )[["estimate"]]
}
...
save(param_unadjusted_runif, file = "data-raw/RObject/param_unadjusted_runif.RData")
save(param_actual_runif, file = "data-raw/RObject/param_actual_runif.RData")
save(param_adjusted_runif, file = "data-raw/RObject/param_adjusted_runif.RData")
...
column_means <- colMeans(comb_para_runif)
