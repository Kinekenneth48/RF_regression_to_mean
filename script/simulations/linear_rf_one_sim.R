...
train_data = train_data %>%
  dplyr::mutate(pred_y = predict(model, data = train_data)$predictions,
                type = "Train") %>%
  dplyr::select(y, pred_y, type)

test_data = test_data %>%
  dplyr::mutate(pred_y = predict(model, data = test_data)$predictions,
                type = "Test") %>%
  dplyr::select(y, pred_y, type)
...
ggplot(df, aes(y, pred_y)) + 
  geom_point(color = "#b2182b",size = 2) + facet_grid(. ~ type)+
  geom_abline(intercept = 0, slope = 1, linetype = "solid", color = "black",lwd = 2)+
  ylab("Estimated Response") + xlab("Actual Response")+
  theme(
